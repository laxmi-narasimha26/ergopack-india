...
export function StrappingMachine({
    ...
    const metalMaterial = useMemo(() => new THREE.MeshStandardMaterial({
        color: '#222222', roughness: 0.4, metalness: 0.8
    }), []);
    const bodyMaterial = useMemo(() => new THREE.MeshStandardMaterial({
        color: bodyColor, roughness: 0.2, metalness: 0.1
    }), [bodyColor]);

    // Smoother Cycle Calculation
    // We want the internal mechanics to respond fluidly
    const lanceExtension = cycleProgress < 0.1 ? 0 :
        cycleProgress < 0.4 ? (cycleProgress - 0.1) * 3.33 :
            cycleProgress < 0.8 ? 1 :
                Math.max(0, 1 - (cycleProgress - 0.8) * 5);

    const lanceBend = cycleProgress < 0.3 ? 0 :
        cycleProgress < 0.5 ? (cycleProgress - 0.3) * 5 :
            cycleProgress < 0.8 ? 1 :
                Math.max(0, 1 - (cycleProgress - 0.8) * 5);


    return (
        <group ref={groupRef} position={position} scale={scale} rotation={rotation ? new THREE.Euler(...rotation) : undefined}>
...
function ChainLanceInstanced({ extension, bend, color }: { extension: number; bend: number; color: string }) {
