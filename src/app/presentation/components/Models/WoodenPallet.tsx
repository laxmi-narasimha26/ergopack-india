  ...
  useFrame(() => {
    if (strapRef.current) {
      if (strapped) {
        ...
        (strapRef.current.material as THREE.MeshStandardMaterial).opacity =
          strapProgress > 0.9 ? 1 : 0;
      } else {
  ...
  return (
    <group ref={groupRef} position={position} scale={scale}>
      ...
      {[0, 1, 2, 3, 4].map((i) => {
        ...
        const boardWidth = i === 0 || i === 4 ? 0.145 : 0.1;
      ...
      {[-0.5, 0, 0.5].map((pos, i) => (
        <mesh
          key={`stringer-${i}`}
          position={[pos * (length - 0.145), height / 2 - 0.022 - 0.011, 0]}
          castShadow
          receiveShadow
        >
      ...
      {[-0.5, 0, 0.5].map((xPos, xi) =>
        [-0.5, 0, 0.5].map((zPos, zi) => (
          <mesh
            key={`block-${xi}-${zi}`}
            position={[xPos * (length - 0.145), 0, zPos * (width - 0.145)]}
            castShadow
            receiveShadow
          >
      ...
      {[-0.5, 0, 0.5].map((zPos, i) => (
        <mesh
          key={`bottom-${i}`}
          position={[0, -height / 2 + 0.011, zPos * (width - 0.145)]}
          castShadow
          receiveShadow
        >
