          ...
          {features.map((feature, i) => {
            ...
            return (
              ...
              >
                ...
                >
                  <span style={{ color: feature.color || '#FFB81C' }}>
              ...
              {testimonials.map((_, i) => (
                <button
                  ...
                  className={`w-2 h-2 rounded-full transition-all ${
                    i === current ? 'bg-[#FFB81C] w-6' : 'bg-white/30'
                  }`}
                />
              ))}
            </div>
      ...
      {visible && (
        ...
        >
                  ...
                  >
                    ...
                    <div
                      className={`w-12 h-12 rounded-full flex items-center justify-center transition-all duration-300 ${
                        isActive ? 'bg-[#FFB81C] scale-125' : 'bg-white/10 hover:bg-white/20'
                      }`}
                    >
                    ...
                    {/* Year */}
                    <p
                      className={`mt-3 text-sm font-bold ${isActive ? 'text-[#FFB81C]' : 'text-white/60'}`}
                    >
                      {event.year}
                    </p>
          ...
          <AnimatePresence mode="wait">
            ...
            >
              <h3 className="text-2xl font-bold text-white mb-2">{events[activeIndex].title}</h3>
              <p className="text-white/60 max-w-2xl mx-auto">{events[activeIndex].description}</p>
            </motion.div>
          </AnimatePresence>
        </motion.div>
      )}
          ...
          {badges.map((badge, i) => {
            ...
            return (
              ...
              >
                ...
                <span className="text-xs text-white/60 text-center">{badge.name}</span>
              </motion.div>
            );
          })}
        </motion.div>
...
export function MouseFollower({ enabled = true }: MouseFollowerProps) {
  ...
  const innerX = useTransform(mouseX, (v) => v - 4);
  const innerY = useTransform(mouseY, (v) => v - 4);
