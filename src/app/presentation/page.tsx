...
export default function PresentationPage() {
  ...
  return (
    <main ref={containerRef} className="relative w-full bg-[#1a1a1a]">
      {' '}
      {/* Darker Cinematic Bg */}
      {/* --- 1. CINEMATIC 3D VIEWPORT --- */}
      <div className="fixed inset-0 z-0 pointer-events-none">
        <Suspense fallback={null}>
          <MainScene currentSection={currentSection} scrollProgress={scrollProgress} />
        </Suspense>
      </div>
      {/* --- 2. FLOATING HUD --- */}
      <header className="fixed top-0 left-0 right-0 z-40 p-8 flex justify-between items-start pointer-events-none mix-blend-screen text-white/80">
        <div>
          <h1 className="text-3xl font-black tracking-tighter">
            ERGOPACK <span className="text-red-600">X</span>
          </h1>
          <span className="text-[10px] font-mono tracking-[0.2em] opacity-80">
